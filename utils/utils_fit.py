...
def fit_one_epoch(model_rpn, model_all, loss_history, callback, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch,
    ...
    val_loss = 0
    with tqdm(total=epoch_step, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as percent_bar:
        for iteration, batch in enumerate(gen):
            ...
            for i in range(len(X)):
                ...
                out_regression.append(Y2)

            loss_class = model_all.train_on_batch([X, np.array(roi_inputs)],
                                                  [Y[0], Y[1], np.array(out_classes), np.array(out_regression)])
            ...
            percent_bar.set_postfix(**{'total': total_loss / (iteration + 1),
                                       'rpn_cls': rpn_cls_loss / (iteration + 1),
                                       'rpn_loc': rpn_loc_loss / (iteration + 1),
                                       'roi_cls': roi_cls_loss / (iteration + 1),
                                       'roi_loc': roi_loc_loss / (iteration + 1),
                                       'lr': K.get_value(model_rpn.optimizer.lr)})
            percent_bar.update(1)

    print('Start Validation')
    with tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as percent_bar:
        for iteration, batch in enumerate(gen_val):
            ...
            percent_bar.set_postfix(**{'total': val_loss / (iteration + 1)})
            percent_bar.update(1)
    ...
    model_all.save_weights(
        'logs/ep%03d-loss%.3f-val_loss%.3f.h5' % (epoch + 1, total_loss / epoch_step, val_loss / epoch_step_val))
