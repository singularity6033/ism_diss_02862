        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            for idx, detection in enumerate(dr_data):
                ...
                for obj in ground_truth_data:
                    if obj["class_name"] == class_name:
                        ...
                        bi = [max(bb[0], bbox_gt[0]), max(bb[1], bbox_gt[1]), min(bb[2], bbox_gt[2]),
                              min(bb[3], bbox_gt[3])]
                        iw = bi[2] - bi[0] + 1
                        ih = bi[3] - bi[1] + 1
                        if iw > 0 and ih > 0:
                            ua = (bb[2] - bb[0] + 1) * (bb[3] - bb[1] + 1) + \
                                 (bbox_gt[2] - bbox_gt[0] + 1) * (bbox_gt[3] - bbox_gt[1] + 1) - iw * ih
                            ov = iw * ih / ua
                            if ov > ovmax:
                                ovmax = ov
                                gt_match = obj
                ...
                if show_animation:
                    ...
                    if ovmax > 0:
                        bbox_gt = [int(round(float(x))) for x in gt_match["bbox"].split()]
                        cv2.rectangle(img, (bbox_gt[0], bbox_gt[1]), (bbox_gt[2], bbox_gt[3]), light_blue, 2)
                        cv2.rectangle(img_cumulative, (bbox_gt[0], bbox_gt[1]), (bbox_gt[2], bbox_gt[3]), light_blue, 2)
                        cv2.putText(img_cumulative, class_name, (bbox_gt[0], bbox_gt[1] - 5), font, 0.6, light_blue, 1,
                                    cv2.LINE_AA)
            ...
            if len(prec) > 0:
                print(text + "\t||\tscore_threshold=0.5 : " + "F1=" + "{0:.2f}".format(F1[score05_idx]) +
                      " ; Recall=" + "{0:.2f}%".format(rec[score05_idx] * 100) +
                      " ; Precision=" + "{0:.2f}%".format(prec[score05_idx] * 100))
            else:
...
def preprocess_gt(gt_path, class_names):
    ...
    for i, image_id in enumerate(image_ids):
        ...
        for line in lines_list:
            ...
            bbox = [left, top, right - left, bottom - top, str(image_id), cls_id,
